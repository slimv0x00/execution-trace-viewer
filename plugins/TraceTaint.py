...
class TraceTaint:
    def __init__(self, ctx):
        ...
        self.initial_symbols = []  # initial taint source symbols (z3 BitVec)
    ...
    def add_taint_register(self, reg_name, var_name=None):
        ...
        self.initial_symbols.append(sym_var)
    ...
    def _update_sp(self, offset_bytes):
        ...
        # Symbolic Update
        if offset_bytes < 0:
            new_sp_s = sp_s - z3.BitVecVal(-offset_bytes, self.ctx.arch_mode)
        else:
            new_sp_s = sp_s + z3.BitVecVal(offset_bytes, self.ctx.arch_mode)
        # Concrete Update
    ...
    def _handle_adc_sbb(self, operands, ip, mode):
        ...
        # 2. [핵심] EFLAGS(CF) 읽기 — 구체값만 필요 (symbolic flags 전체를 더하면 수식 오염)
        flags_reg = 'rflags' if self.ctx.arch_mode == 64 else 'eflags'
        flags_c, _, _ = self.ctx.get_register(flags_reg)
        ...
        # 4. 연산 수행 (Symbolic)
        # CF는 1비트 구체값으로 사용. symbolic flags 전체를 더하면 수식이 오염됨.
        dst_bits = d_s.size()
        mask = (1 << dst_bits) - 1
        cf = (flags_c >> 0) & 1
        cf_sym = z3.BitVecVal(cf, dst_bits)

        new_sym = None
        if mode == 'ADC':
            new_sym = z3.simplify(d_s + s_s + cf_sym)
            new_conc = (d_c + s_c + cf) & mask
        else:  # SBB
            new_sym = z3.simplify(d_s - s_s - cf_sym)
            new_conc = (d_c - s_c - cf) & mask
    ...
    def _handle_rotate(self, operands, ip, mode):
        ...
        # Concrete 값 계산
        dst_bits = d_s.size()
        n = c_c % dst_bits if dst_bits > 0 else 0
        mask = (1 << dst_bits) - 1
        if mode == 'ROL':
            new_conc = 0 if n == 0 else ((d_c << n) | (d_c >> (dst_bits - n))) & mask
        else:  # ROR
            new_conc = 0 if n == 0 else ((d_c >> n) | (d_c << (dst_bits - n))) & mask
