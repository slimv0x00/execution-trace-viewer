...
class TraceAdimehtTIGER:
    ...
    def _try_classify_mem_operand(self, mem_op, ip):
        ...
        # Pattern 2: base=VBR, index present
        elif base_root == self._vbr_reg and index:
            idx_state = self._get_state(index)
            if idx_state and idx_state[0] == 'vm_val':
                # index has ('vm_val', role) → next depth
                ...
                # [VBR + any_reg]: arbitrary index register → VB (depth 1)
                vbr_c, _, _ = self.ctx.get_register(self._vbr_reg)
                offset = (addr_c - vbr_c) & mask
                classification = ('VB', 1, offset)
    ...
    # Public Queries
    # =========================================================================
    def _make_label(self, elem: tuple) -> str:
        """Return 'ROLE_depth_0xNN' label string from a vm_elements tuple."""
        role, depth, offset = elem
        return f"{role}_{depth}_{hex(offset)}"
    ...
    def get_vm_elements_snapshot(self):
        ...
        """
        result = []
        for addr, elem in sorted(self.vm_elements.items()):
            label = self._make_label(elem)
            result.append({
