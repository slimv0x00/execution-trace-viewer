...
class PluginMvAdimeht(IPlugin):
    ...
    def get_register_value_from_trace(self, trace, reg_name):
        _reg_index = {
            'eax': 0, 'ebx': 1, 'ecx': 2, 'edx': 3, 'ebp': 4, 'esp': 5, 'esi': 6, 'edi': 7,
            'eip': 8, 'eflags': 9,
            'ax': 0, 'bx': 1, 'cx': 2, 'dx': 3, 'bp': 4, 'sp': 5, 'si': 6, 'di': 7,
            'ah': 0, 'bh': 1, 'ch': 2, 'dh': 3,
            'al': 0, 'bl': 1, 'cl': 2, 'dl': 3,
        }
    ...
    def get_dst_src(self, trace, instruction):
        _dst = []
        _src = []
        return _dst, _src
    ...
    def run_taint(self, trace):
        ...
        for _inst in _instructions:
            ...
            _dst, _src = self.get_dst_src(trace, _inst)
    ...
    def execute(self, api: Api):
        ...
        _input_dlg_data = [
            ...
            {'label': 'Target index(#)', 'data': 2},
            {'label': 'Target operand', 'data': 'edx'},
            {'label': 'TTL (no limit, -1)', 'data': 100},
        ]
